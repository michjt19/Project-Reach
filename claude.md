...
- Hosted on Vercel at thereachcommunity.com (custom domain configured in Vercel dashboard)
- Repository: github.com/michjt19 (authenticated as michjt19) — Vercel auto-deploys on push to main
- Build command: `npm run build` (Next.js build + next-sitemap postbuild)
- Environment variables set in Vercel dashboard: NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, RESEND_API_KEY, VOLUNTEER_NOTIFY_EMAIL
- Local env template: .env.local.example (copy to .env.local for local dev)
- No staging environment — Vercel preview deployments serve as staging for feature branches

TECH STACK:

- Next.js 14 (App Router), React 18, TypeScript 5
- Tailwind CSS 3 with CSS custom property tokens (globals.css) and darkMode: 'class'
- Sanity CMS (optional — site works with static fallbacks if env vars not set)
- Do not introduce additional JS frameworks or UI libraries without explicit approval
- New pages go in app/ as Server Components by default; use 'use client' only when interactivity requires it
- New styles go in globals.css (tokens) or as Tailwind classes — no CSS-in-JS, no new stylesheets

FILE STRUCTURE:

- Pages: app/**/page.tsx (App Router — one directory per route)
- API routes: app/api/**/route.ts (currently: POST /api/volunteer)
- Layout components: components/layout/ (Nav.tsx, Footer.tsx, JsonLd.tsx)
- UI components: components/ui/ (client components: BlogIndex, Carousel, AnimatedCounter, ResourceDirectory, CookieConsent)
- Data layer: lib/sanity.ts (GROQ queries + types), lib/seo.ts (metadata helper), lib/search.ts (Lunr)
- Static content fallbacks: content/blog/posts.ts (blog), content/resources.json, content/team.json
- Sanity Studio: sanity/ (separate sub-project with its own package.json — run separately)
- Public assets: public/assets/ (favicon.png, logo.svg, reach-logo.png), public/og-image.jpg
- Sitemap: public/sitemap.xml + public/sitemap-0.xml (auto-generated by next-sitemap on build)

EXISTING INTEGRATIONS (do not duplicate or conflict with these):

- Live chat: Tawk.to (widget ID: 6843b2759ed8c2190a6d55fb/1jjvkm9qn) — loaded via next/Script in app/layout.tsx, triggered via openTawkChat() exported from components/layout/Nav.tsx
- Volunteer form: Resend API via /api/volunteer/route.ts — sends email to VOLUNTEER_NOTIFY_EMAIL and confirmation to applicant
- Community forum: Discourse at https://thereachcommunity.discourse.group/ — linked externally
- Analytics: Google Analytics 4 (measurement ID: G-S8M7YHYFMV) — loaded via next/Script in app/layout.tsx with consent-first pattern (denied by default, granted via CookieConsent component)
- Fonts: Playfair Display (headings) and Lora (body) via next/font/google — self-hosted, no external CDN request
- CMS: Sanity (project ID in env vars) — graceful fallback to static content if not configured

BRANDING:

- Primary color: #2D4A3E (dark sage green) — defined as --color-primary in globals.css
- Accent: #C4622D (burnt orange) — defined as --color-accent in globals.css
- Typography: Playfair Display (--font-heading) for headings, Lora (--font-body) for body
- Tone: warm, calm, trustworthy, non-clinical
- Dark mode: Tailwind darkMode: 'class' — toggled by adding 'dark' class to <html>; all tokens redefined in .dark {} block in globals.css
...
- next-sitemap.config.js or public/robots.txt (controls crawl directives and sitemap generation)
- Google Analytics measurement ID
- Any crisis resource contact information
- The Tawk.to widget embed code or widget ID
- package-lock.json — do not run npm install unless package.json changed; use npm ci to restore node_modules
